...
企业微信入站消息统一先由 `data/wecom_intent.py` 调用配置中的模型理解意图，
不再用买入、卖出、股票代码等关键词抢先分流。模型输出普通问答、实际成交回填、
撤销本地记录、展示选股报告、重置会话或澄清问题中的一种动作。
当前正文、引用、图片及最近自己的对话分开提供；引用中的建议、示例和历史指令本身不授权新成交。

普通问答继续进入原有持久会话。实际成交使用模型提取的结构化参数，执行层只校验权限、
字段、记录状态和幂等性；模型不可用或输出不合法时不退回关键词入账。
撤销只将指定本地记录标记为 `cancelled`，保留成交字段与消息审计信息；影子账户重算时排除该记录。
所有操作均不向券商发送订单。现有飞书旧入口与命令行文本解析接口不属于企业微信的分流链路。

每次意图判断的输入、输出、模型和提示词版本保存在 `data/agent_runs/wecom-routing/`。
智能机器人每条消息启动独立 Python 处理进程，因此更新代码后下一条消息即使用新流程。
